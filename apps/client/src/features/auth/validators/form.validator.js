...
export const Validators = Object.freeze({
	...
	onlyAlphaNumeric: (fieldName, value, options = {}) => {
		...
		const escapedSpecialChars = allowSpecialChars.replace(
			/[.*+?^${}()|[\]\\]/g,
			"\\$&"
		);
	...
	dateWithinLimits: (fieldName, value, options = {}) => {
		...
		const ageInYears = Math.floor(
			(today - inputDate) / (365.25 * 24 * 60 * 60 * 1000)
		);
	...
	addressValidator: (fieldName, value, options = {}) => {
		...
		const minLineLength =
			options.minLineLength || DEFAULTS.addressLineMinLength;
	...
	fileValidator: (fieldName, file, options = {}) => {
		...
		const allowedExtensions =
			options.allowedExtensions || DEFAULTS.allowedFileExtensions;
